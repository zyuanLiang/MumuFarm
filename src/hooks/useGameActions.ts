...
export const useGameActions = ({
  ...
  const handleHarvest = useCallback(
    (plotId: number) => {
      ...
      if (gameState.activeSkin === 'cyber' && result.goldMultiplier > 1) {
        onCyberAurora?.();
      }
    },
    [gameState, onCyberAurora, onDivineFlare, onHarvestFeedback, setGameState],
  );
  ...
  const handleDraw = useCallback((count = 1) => {
    ...
    if (
      sessionResult.summaryEffect.triggerRainbow ||
      (gameState.activeSkin === 'cyber' && sessionResult.rewards.some((reward) => reward.type === 'prop'))
    ) {
      onCyberAurora?.();
    }
