...
export default function App() {
  const {gameState, setGameState, xpToNextLevel, levelUpEvent, clearLevelUpEvent} = useGameState();
  ...
  return (
    <div
      className="game-world pt-20 pb-48 px-4 transition-all duration-1000 min-h-screen overflow-x-hidden"
      style={
        currentSkin.bgImage
          ? {
          ...
          : {
              background: `linear-gradient(to bottom, #BAE6FD 0%, #BAE6FD 30%, ${currentSkin.color} 30%, ${currentSkin.color} 100%)`,
            }
      }
    >
      {currentSkin.bgImage && <div className="absolute inset-0 bg-black/10 pointer-events-none -z-10" />}
          ...
          <AnimatePresence>
            {isSidebarOpen && (
              <motion.div
                ...
                className="fixed right-16 top-24 sm:right-24 sm:top-24 bottom-32 w-56 sm:w-72 flex flex-col gap-3 sm:gap-4 z-[30]"
              >
                <AnimatePresence>
                  {gameState.activeBuffs.length > 0 && (
                    <motion.div
                      ...
                      className="vibrant-card !bg-amber-600 border-amber-800 p-4 space-y-3"
                    >
                      <div className="text-xs font-black text-amber-100 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Sparkles size={14} /> 增益激活中
                      </div>
                      {gameState.activeBuffs.map((buff) => {
                        const remaining = Math.max(0, Math.floor((buff.expiresAt - Date.now()) / 1000));
                        const mins = Math.floor(remaining / 60);
                        const secs = remaining % 60;
                        return (
                          <div
                            key={buff.id}
                            className="bg-amber-700/50 rounded-2xl p-3 flex items-center justify-between shadow-inner"
                          >
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] font-bold text-amber-200">
                                {mins}:{secs.toString().padStart(2, '0')} 剩余
                              </span>
                            </div>
                            <div className="text-white font-black text-lg drop-shadow-md">x{buff.multiplier}</div>
                          </div>
                        );
                      })}
                    </motion.div>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                  <div className="vibrant-card !bg-white p-5">
                    <div className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 border-b-2 border-slate-100 pb-3 flex items-center gap-2">
                      <Package size={16} className="text-blue-500" /> 道具仓库
                    </div>
                    <div className="grid grid-cols-1 gap-3">
                      {gameState.inventory.length === 0 && (
                        <div className="text-xs text-slate-300 font-bold py-4 text-center">空空如也</div>
                      )}
                      {gameState.inventory
                        ...
                        .map((item) => (
                          <div
                            key={item.id}
                            className="vibrant-card !bg-slate-50 !border-slate-200 !p-3 group hover:border-blue-400 transition-colors"
                          >
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-xs font-black text-blue-500 bg-blue-50 px-2 py-1 rounded-lg">
                                x{item.count}
                              </span>
                            </div>
                            <p className="text-[9px] font-medium text-slate-400 mb-3 leading-tight italic">
                              "{item.description ?? '收藏到仓库中，等待后续使用。'}"
                            </p>
                            {item.type === 'prop' ? (
                              <button
                                onClick={() => handleUseProp(item.id)}
                                className="w-full bg-gradient-to-b from-blue-400 to-blue-600 text-white text-[10px] font-black py-2 rounded-xl shadow-[0_4px_0_#1e40af] active:translate-y-[2px] active:shadow-none transition-all"
                              >
                                立即使用
                              </button>
                            ) : (
                              <div className="w-full rounded-xl bg-slate-100 text-slate-500 text-[10px] font-black py-2 text-center">
                                已收纳
                              </div>
                            )}
                          </div>
                  ...
                  <div className="vibrant-card !bg-emerald-50 !border-emerald-500 p-5">
                    <div className="text-xs font-black text-emerald-600 uppercase tracking-widest mb-4 border-b-2 border-emerald-200 pb-3">
                      赛季任务
                    </div>
